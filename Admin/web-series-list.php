...
<div class="container">
<form class="d-md-flex ms-4 search" method="post" action="web-search.php">
...
$new = mysqli_query($con, "SELECT * FROM `add_webs` ORDER BY `add_webs`.`id` DESC  LIMIT {$offset},{$limit}");
if($new){
...
<div class="btn-toolbar d-flex justify-content-center" role="toolbar">
  ...
  if(mysqli_num_rows($new11) > 0){
    ...
    for($i = 1; $i <= $page; $i++){
      ...
      echo '<li type="button" id="pag" class="btn btn-danger '.$active.'"><a href="web-series-list.php?page='.$i.'">'.$i.'</a></li>';
    }
    echo '</nav>';
  }
  ?>
